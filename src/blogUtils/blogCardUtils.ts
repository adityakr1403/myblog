...
const getFormattedDate = (post: blogger_v3.Schema$Post | undefined) => {
    const date = new Date(post?.published ?? '');
    const options = {year: 'numeric', month: 'long', day: '2-digit'} as Intl.DateTimeFormatOptions;
    return new Intl.DateTimeFormat('en-US', options).format(date);
}

const getImageURL = (post: blogger_v3.Schema$Post | undefined) => {
    const imageURLs = [];
...
const getDescription = (post: blogger_v3.Schema$Post | undefined) => {
